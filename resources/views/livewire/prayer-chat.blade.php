<div wire:poll.5s="refreshMessages" class="bg-white border border-gray-100 shadow-xl rounded-2xl overflow-hidden flex flex-col h-[500px]">
    <div class="px-6 py-4 border-b border-gray-100 bg-white">
        <h3 class="text-base font-bold text-soul-indigo flex items-center gap-2">
            ...
            Acompañamiento en oración
        </h3>
        <p class="text-xs text-gray-500 mt-1 font-medium">Este es un espacio seguro y confidencial. Solo los intercesores asignados y tú pueden ver estos mensajes.</p>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-6 py-4 space-y-6 bg-gray-50" id="chat-messages-container">
            ...
            <div class="flex {{ $mine ? 'justify-end' : 'justify-start' }} animate-fade-in-up" style="animation-duration: 0.3s;">
                <div class="flex items-end gap-3 max-w-[80%] {{ $mine ? 'flex-row-reverse' : 'flex-row' }}">
                    ...
                    <div class="flex flex-col {{ $mine ? 'items-end' : 'items-start' }}">
                        ...
                        <div class="px-4 py-2.5 shadow-sm text-sm whitespace-pre-wrap break-words {{ $mine ? 'bg-soul-indigo text-white rounded-2xl rounded-br-sm' : 'bg-white border border-gray-200 text-soul-indigo rounded-2xl rounded-bl-sm' }}">{{ $message->body }}</div>
        ...
        @empty
            <div class="h-full flex flex-col items-center justify-center text-center px-4">
                ...
                <p class="text-sm font-bold text-soul-indigo">El chat está vacío</p>
                <p class="text-xs text-gray-500 mt-2 max-w-xs font-medium leading-relaxed">Escribe tu primer mensaje. Un intercesor te responderá pronto para acompañarte.</p>
            </div>
        @endforelse
    </div>
